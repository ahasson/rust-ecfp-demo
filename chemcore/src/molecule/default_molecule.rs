...
use super::{Atom, Molecule, Node, Bond};
...
impl Molecule for DefaultMolecule {
    ...
    fn get_bond(&self, sid: usize, tid: usize) -> Result<&Bond, GraphError> {
        let source = self.node_for(sid)?;
        ...
        match source.bonds.iter().find(|bond| bond.tid == tid) {
            Some(bond) => Ok(bond),
            None => Err(GraphError::UnknownId(tid)),
        }
    }
}
...
mod size {
    ...
    fn one_edge() {
        let molecule = DefaultMolecule::new(vec![
            Node {
                atom: Atom::default(),
                bonds: vec![Bond::new(2, None, 1, 0)],
            },
            Node {
                atom: Atom::default(),
                bonds: vec![Bond::new(2, None, 0, 0)],
            },
        ]);
...
mod neighbors {
    ...
    fn known_id() {
        let molecule = DefaultMolecule::new(vec![
            Node {
                atom: Atom::default(),
                bonds: vec![Bond::new(2, None, 1, 0), Bond::new(2, None, 2, 0)],
            },
            Node {
                atom: Atom::default(),
                bonds: vec![Bond::new(2, None, 0, 0)],
